...
void Textbox::set_text(const c8* str) noexcept {
  this->text = str;
...
void Textbox::update_texture() noexcept {
...
void Textbox::unfocused() noexcept {
  this->focused = false;
  this->tex = std::move(renderer::create_text(this->text.c_str()));
...
void Textbox::key_input(event::KeyPress& keypress) noexcept {
  if (!this->focused) {
...
void Textbox::update(f32 _delta) noexcept {

...
void FileModal::init(fvec window_size, const Renderer& renderer) noexcept {
  ...
  this->title_rect.size = {FILE_MODAL_WIDTH, renderer.get_text_height() + 8.0F};
  fvec off = this->pos + fvec{4.0F, 4.0F};

  this->title_text.pos = off;
  this->title_text.set_text(cfg::locale::TextId::NEW_FILE, renderer);
...
void FileModal::locale_updated(const Renderer& renderer) noexcept {
  this->title_text.locale_updated(renderer);
...
void FileModal::render(const Renderer& renderer) const noexcept {
  ...
  // Texts
  this->title_text.render(renderer);
